...
1) post all of Net_Assets. Post Opening Balances for each member.
...
Some musing: notice how the program would not be able to generate the fund's P&L report from the state of the ledger after allocation transactions are posted. In what we've been doing so far (risera etc), this is sidestepped by projecting (like with an unbalanced journal entry) the total Comprehensive_Income into Equity-Current_Earnings after all transactions are processed. 

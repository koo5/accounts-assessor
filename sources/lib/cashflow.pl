...
*/
 ...
 add_entry_balance_desc(_Sd, Entry, Balance, Column, Text, Type) :-
	!format_vec(texto, [], _, xxx, 'https://rdf.lodgeit.net.au/v1/kb#debit', Balance, Balance_Text),
...
*/
 ...
 cashflow(
	...
	(	abrl('Cash_and_Cash_Equivalents', Root)
	->	(
	...
	;	%!make_report_entry('cashflow not calculated', [], Entry),
		Entries = []
	).